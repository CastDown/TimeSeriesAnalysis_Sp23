...
>>>>>>> 00f2f28160d8b288489269538359ffb16426ee26
>>>>>>> 480129d9b6cdfc6c66142c0976e42a01d8e2963b
```
...
***Answers***: among three series, hydroelectric power consumption has the strongest seasonality. Therefore, I will use this series to analyze its seasonality. From the results, we can see most seasonal dummies are statistically significant at 5% or 1% confidence level, which demonstrates the strong seasonality of this series. 

```{r}

dummies <- seasonaldummy(ts_renewables[,1]) #create seasonal dummy variables

lm_season <- lm(formula = ts_renewables[,3] ~ dummies) #store regression result to a list

summary(lm_season)

cf_season <- lm_season$coefficients[2:12] #store slopes of seasonal dummy variables

beta0 <- lm_season$coefficients[1] #store intercept of the regression result
...
ggplot(data = lm_season, aes(y = lm_season$residuals, x = t)) + 
  geom_line() +
  geom_line(aes(y = renewables_sub$`Hydroelectric Power Consumption`), col = "green")

#residuals in regression results are already the deseasoned series we want. So I use residuals to plot. Below is the code of using coefficients to calculate the deseasoned series:
...
***Answer***: The plots changed. Compared to previous acf and pacf, there is no strong seasonality now. ACF shows a downward trend. Most PACF are not significant, which may imply stationarity. 
...
plot(y = lm_season$residuals, x = t, type = "l", 
     main = "Plot of Deseason Hydroelectric /n Energy Consumption")

acf(x = ts_deseason_hydro, lag.max = 40, xlab = "Time", ylab = "ACF",
    main = "Autocorrelation of Deseason /n Hydroelectric Energy Consumption", 
    ylim = c(-1, 1)
    )

pacf(x = ts_deseason_hydro, lag.max = 40, xlab = "Time", ylab = "PACF",
    main = "Partial Autocorrelation of Deseason /n Hydroelectric Energy Consumption", 
    ylim = c(-1, 1)
    )
```
