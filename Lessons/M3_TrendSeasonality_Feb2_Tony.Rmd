...
inflow_data <- subset(inflow_data, !is.na(inflow_data$HP1))
...
for (i in 2:(nhydro+1)){
  par(mfrow=c(1,2))
  acf(ts_inflow_data[,i], lag.max = 40, 
      main = paste("HP", i), ylim = c(-1, 1))
  pacf(ts_inflow_data[,i], lag.max = 40, 
       main = paste("HP", i), ylim = c(-1, 1))
}
...
Let's identify and remove trend component like we learned on the recorded videos for M4. You start by fitting a linear model to $Y_t=\beta_0+\beta_1*t+\epsilon_t$. 
