...
best_model_index <- which.min(seas_scores[,"RMSE"]) 
  #output the row index of the one with the lowest RMSE
cat("The best model by RMSE is:", row.names(seas_scores[best_model_index,]))  
  #output a sentence with the best model
...
```{r}
#Alternative way to plot with autoplot and autolayer
ts_electricity_price_full <- ts(
  electricity_price_processed[,"All.sectors"],
  start=c(year(electricity_price_processed$Month[1]),month(electricity_price_processed$Month[1])),
  frequency=12) 

autoplot(ts_electricity_price_full) +
    autolayer(SARIMA_for,series="SARIMA",PI=FALSE) +
    autolayer(MEAN_seas,series="Arithmetic Mean",PI=FALSE) +
    autolayer(SNAIVE_seas,series="Seasonal Naive",PI=FALSE) +
