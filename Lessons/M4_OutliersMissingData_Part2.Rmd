...
decompose(clean_wind_data, type = "additive") %>% plot()

#Ex2: Store the three component in separate vector
decom_stats <- decompose(clean_wind_data, type = "additive")  
decom_stats_df <- data.frame(seasonal = decom_stats$seasonal, 
                             trend = decom_stats$trend,
                             randome = decom_stats$random
                            )

#Ex3: Create one data frame with all three vector and the Date columns from wind_data
decom_stats_df <- cbind(decom_stats_df, wind_data$DATE)
...
```
The first 6 observations and the last 6 observations are omitted because moving average method needs 6 obs before and 6 obs after to calculate the trend and seasonality. Thus, the first and last 6 obs don't have enough obs before or after them. Therefore, they are omitted. 
